...
const RetirementShow = () => {

  const [members, setMembers] = useState([]);

  //show data
  const fetchMembers = async () => {
    const resShow = await fetch(`${apiUrl}/retirements`, {
      'method': 'GET',
      'headers': {
  ...
  const deleteMember = async (id) => {
    ...
    const resShow = await fetch(`${apiUrl}/retirements/${id}`, {
      'method': 'DELETE',
      'headers': {
        'Content-type': 'application/json'
      }
          ...
          <div className="p-4">
            <div className="mx-auto max-w-7xl">
              <div className="flex items-center justify-between mb-6">
                <h1 className="text-xl font-bold">Retirements</h1>
                <Link to={'/backend/websitechanges/retirement/create'} > <button className="px-4 py-2 text-white bg-blue-500 rounded shadow-md hover:bg-blue-600">
                  ADD NEW
                </button></Link>
              </div>
