...
const CurrentCreate = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axios.post(`${apiUrl}/current_bearers`, formData, {
        headers: {
      ...
      if (response.data.status == true) {
        toast.success("added");
        nevigate('/backend/websitechanges/current')
      ...
      <div className="flex items-center justify-center ">
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
          <div className="relative w-1/2 p-6 bg-white rounded-lg shadow-lg md:w-3/5 lg:w-2/5">
            ...
            <h2 className="mb-4 text-2xl font-bold place-self-center"><Link to={'/backend/websitechanges/current'}>Current Bearers</Link></h2>
            ...
            <form className="flex flex-col items-center justify-center" onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="flex justify-end pt-5 space-x-4 md:col-span-2">
                <Link to={'/backend/websitechanges/current'}> <button
                  type="button"
